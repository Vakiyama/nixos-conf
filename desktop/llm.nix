{ pkgs-unstable, lib, pkgs, ... }:
let
  modelDir = "/var/lib/models";
  modelFile = "Qwen3.6-27B-Q3_K_M.gguf";
  ...
  llamaSwapConfig = (pkgs-unstable.formats.yaml { }).generate "llama-swap-config.yaml" {
    ...
    models = {
      "qwen3.6:27b" = {
        cmd = "${llama-server} --port \${PORT} -m ${modelPath} -ngl 20 --flash-attn on --cache-type-k q8_0 --cache-type-v q8_0 --ctx-size 32768 --jinja --temp 0.6 --top-p 0.95 --top-k 20 --no-webui";
        ttl = 600;
      };
    };
  };
in
{
      ...
      ExecStart = pkgs.writeShellScript "download-qwen36" ''
        if [ ! -f "${modelPath}" ]; then
          ...
          aria2c \
            ...
            --dir="${modelDir}" \
            --out="${modelFile}" \
            "https://huggingface.co/unsloth/Qwen3.6-27B-MTP-GGUF/resolve/main/${modelFile}?download=true"
        else
  ...
  systemd.services.open-webui = {
    ...
    environment = {
      OPENAI_API_BASE_URL = "http://127.0.0.1:${llamaSwapPort}/v1";
